...
>
> ![ComplianceScout Architecture](architecture_diagram.png)
...
├── app.py                    # Streamlit web UI (dashboard + chat interface)
...
### 6. Run the web UI (Streamlit dashboard)

ComplianceScout includes a professional web interface for visual
interaction with the agent — no terminal required.

```bash
streamlit run app.py
```

This opens a browser dashboard at `http://localhost:8501` with:
- **Sidebar controls** — Contractor License #, default ZIP Code, Session ID,
  AgentCore trace viewer, and clear-chat.
- **Chat interface** — conversational UI backed by the same Strands Agent
  (`build_agent`) and custom tools used by the CLI.
- **Permit draft rendering** — when the agent drafts a permit, it's rendered
  inside a styled card with a clear
  **"STATUS: DRAFT — REQUIRES HUMAN SIGNATURE"** badge.
